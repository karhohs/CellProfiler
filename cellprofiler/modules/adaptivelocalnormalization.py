...
class AdaptiveLocalNormalization(cellprofiler.module.ImageProcessing):
    ...
    def calculate_aln_image(self, img, radius_image, threshold_std, radius_max):
        ...
        """
        aln_image = numpy.zeros_like(img, dtype="float")
        ...
        for r in radii:
            if self.strel_shape.value == "disc":
                if self.radius_method.value == "std":
                    ...
                    img_std = scipy.ndimage.filters.generic_filter(img.astype("float"), numpy.std, footprint=strel)
                    img_std[img_std < t_std] = t_std
                    img_mean = scipy.ndimage.filters.generic_filter(img.astype("float"), numpy.mean, footprint=strel)
                    img_norm = (img.astype("float") - img_mean) / img_std
                    aln_image[radius_image == r] = img_norm[radius_image == r]
                elif self.radius_method.value == "cv":
                    strel = skimage.morphology.disk(r)
                    img_std = scipy.ndimage.filters.generic_filter(img.astype("float"), numpy.std, footprint=strel)
                    img_mean = scipy.ndimage.filters.generic_filter(img.astype("float"), numpy.mean, footprint=strel)
                    img_std[img_std < t_std] = t_std
                    img_norm = (img.astype("float") - img_mean) / img_std
                    aln_image[radius_image == r] = img_norm[radius_image == r]
            elif self.strel_shape.value == "square":
                if self.radius_method.value == "std":
                    img_std = self.uniform_filter_std(img.astype("float"), r)
                    img_std[img_std < t_std] = t_std
                    img_mean = self.uniform_filter_mean(img.astype("float"), r)
                    img_norm = (img.astype("float") - img_mean) / img_std
                    aln_image[radius_image == r] = img_norm[radius_image == r]
                elif self.radius_method.value == "cv":
                    img_std = self.uniform_filter_std(img.astype("float"), r)
                    ...
                    img_norm = (img.astype("float") - img_mean) / img_std
                    aln_image[radius_image == r] = img_norm[radius_image == r]
        aln_image = skimage.exposure.rescale_intensity(aln_image, out_range = numpy.uint16)
        aln_image = numpy.uint16(aln_image)
        return aln_image
        ...
        for r in radii:
            if self.strel_shape.value == "disc":
                ...
                if self.radius_method.value == "std":
                    ...
                    img_std = self.uniform_filter_std(img.astype("float"), r)
                    img_mean = self.uniform_filter_mean(img.astype("float"), r)  # remove
                    img_cv = img_std / (img_mean + 1) # the +1 is okay if uint8 or uint16
                    img_bool = img_cv <= t_cv
                    radius_image[img_bool] = r
        return radius_image
    ...
    def create_settings(self):
        ...
        self.strel_shape = cellprofiler.setting.Choice(
            "Structuring Element Shape", ["square", "disc"], doc="""
